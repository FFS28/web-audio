    ...
    return {
        ...
        'lint-src': {
            cmd: `htmlhint --rules ${convertConfig(documentConfig)} 'src/**/index.html' && htmlhint --rules ${convertConfig(
                templateConfig
            )} 'src/app/**/*.component.html' && ng lint web-audio-conference-2016 --type-check`
        },
        'lint-test': {
        ...
        'verify': {
            cmd:
                "bundle-buddy build/web-audio-conference-2016/*.js.map && grep -r build/**/*.map -e '/environments/environment.ts'; test $? -eq 1"
        }
    };
};
